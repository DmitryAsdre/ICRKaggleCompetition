...
class CFG:
    ...
    additional_params_hyperopt = {'task_type' : 'GPU', 'devices' : '1'}
...
class CatboostBaggingROS:
    def __init__(self, catboost_params, additional_params={'task_type' : 'GPU', 'devices' : '0'}, n_bag=100, random_state=42):
        ...
        self.ros = SMOTE(random_state=42)
...
def objective(params):
    ...
    clf = CatboostBaggingROS(params, n_bag=CFG.n_bag, random_state=CFG.random_state, additional_params=CFG.additional_params_hyperopt)
...
if __name__ == '__main__':
    ...
    #train = pd.DataFrame(index=train.index, data=mean_imp.fit_transform(train), columns=train.columns)
    train = train[['Class']]
    ...
    with open(os.path.join(CFG.MODEL_PATH, 'cb_meta_model_binary_smote.pickle'), 'wb') as w:
        pickle.dump(clf, w)
