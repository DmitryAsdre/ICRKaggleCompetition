...
class CFG:
    ...
    model_name = f'Catbost_bagging_tsne_kmeans'
...
if __name__ == '__main__':
    ...
    CFG.additional_params_hyperopt['devices'] = str(sys.argv[2])
    ...
    X_train_kmeans = create_kmeans(X_train, CFG.n_kmeans, CFG.letters)
    X_train_tsne = create_tsne(X_train, CFG.n_tsne, CFG.letters)
    
    X_train = X_train.join(X_train_kmeans).join(X_train_tsne)
    ...
    with open(os.path.join(CFG.PATH_TO_SAVE, CFG.exp_name, f"{CFG.model_name}_{CFG.letter}.pickle"), 'wb') as w:
        pickle.dump(clf, w)
