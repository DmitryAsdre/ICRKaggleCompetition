...
class CatboostBaggingROSGreeks:
    def __init__(self, catboost_params, model_name, additional_params={'task_type' : 'GPU', 'devices' : '0'},
                    n_bag=100, random_state=42):
...
if __name__ == '__main__':
    ...
    X_train, y_train, y_alpha = train.drop(['Class', 'Alpha'], axis=1), train.Class, train.Class
    clf.fit(X_train, y_train, y_alpha)
    
    print(clf.losses[-1])
    
    with open(os.path.join(CFG.MODEL_PATH, 'cb_meta_model.pickle'), 'wb') as w:
        pickle.dump(clf, w)
